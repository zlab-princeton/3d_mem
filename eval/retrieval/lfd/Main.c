// Headless/GLUT dual build:
//   Cluster (headless):   gcc -DHEADLESS_OSMESA ... -lOSMesa -lGLU -lm
//   Laptop (GLUT):        gcc ... -lglut -lGL -lGLU -lm

#ifdef HEADLESS_OSMESA
  #include <GL/osmesa.h>
  #include <GL/gl.h>
  #include <GL/glu.h>
#else
...
#define QUANT8             256
#define FD_SCALE           2
#define CIR_SCALE          2.318181818
#define ECC_SCALE          25.5
...
#ifdef HEADLESS_OSMESA
// -------- OSMesa bootstrap --------
static OSMesaContext g_ctx = NULL;
static void *g_fb = NULL;

static int create_osmesa_context(int w, int h) {
    g_ctx = OSMesaCreateContextExt(OSMESA_RGBA, /*depthBits*/24, /*stencil*/0, /*accum*/0, NULL);
    if (!g_ctx) { fprintf(stderr, "OSMesaCreateContextExt failed\n"); return 0; }
    g_fb = malloc((size_t)w * (size_t)h * 4); // RGBA8
    if (!g_fb)   { fprintf(stderr, "malloc framebuffer failed\n"); return 0; }
    if (!OSMesaMakeCurrent(g_ctx, g_fb, GL_UNSIGNED_BYTE, w, h)) {
        fprintf(stderr, "OSMesaMakeCurrent failed\n");
        return 0;
    }
    // Keep OpenGL's conventional origin (bottom-left = first row).
    OSMesaPixelStore(OSMESA_Y_UP, 1);
    winw = w; winh = h;
    return 1;
}

static void destroy_osmesa_context(void) {
    if (g_ctx) { OSMesaDestroyContext(g_ctx); g_ctx = NULL; }
    free(g_fb); g_fb = NULL;
}
#endif // HEADLESS_OSMESA

void FindCenter(unsigned char *srcBuff, int width, int height, double *CenX, double *CenY)
{
    int x, y;
    unsigned char *pImage = srcBuff;
    int maxX = -1, maxY = -1, minX = INT_MAX, minY = INT_MAX;

    for (y=0; y<height; y++)
    for (x=0; x<width;  x++, pImage++) {
        if (*pImage < 255) {
            if (x > maxX) maxX = x;
            if (x < minX) minX = x;
            if (y > maxY) maxY = y;
...
void display(void)
{
    ...
    for (int i = 0; i < NumTri; i++) {
        ...
        glBegin(GL_POLYGON);
        for (int j = 0; j < triangle[i].NodeName; j++) {
            glVertex3d(vertex[triangle[i].v[j]].coor[0],
                       vertex[triangle[i].v[j]].coor[1],
                       vertex[triangle[i].v[j]].coor[2]);
        }
        glEnd();
    }
    glPopMatrix();
...
void renderingFun()
{
    ...
    FILE *fpt, *fpt2, *fpt3, *fpt4, *fpt_art_q8, *fpt_art_q4, *fpt_fd_q8, *fpt_fd, *fpt_cir_q8, *fpt_ecc_q8;
    ...
    uint64_t CompactColor[ANGLE][CAMNUM];
    uint64_t dest_CompactColor[ANGLE][CAMNUM];
    ...
    double QuantTable[17] = {
        0.000000000, 0.003585473, 0.007418411, 0.011535520,
    ...
    for (i=0; i<CAMNUM; i++) {
        srcBuff[i]   = (unsigned char*) malloc (winw * winh);
        ColorBuff[i] = (unsigned char*) malloc (3 * winw * winh);
    }
    YuvBuff  = (unsigned char*) malloc (3 * winw * winh);
    EdgeBuff = (unsigned char*) malloc (winw * winh);

    total = winw * winh;
    Contour     = (sPOINT*)        malloc(total * sizeof(sPOINT));
    ContourMask = (unsigned char*) malloc(total);
    ...
    for (j=0; j<CAMNUM; j++) {
        ...
        for (int r=1; r<ART_RADIAL; r++, k++) {
            int itmp = (int)(QUANT8 * src_ArtCoeff[i][j][p][r]);
            q8_ArtCoeff[i][j][k] = (itmp>255) ? 255 : itmp;
        }
    ...
    fpt = fopen(filename, "wb"); if (!fpt) { printf("Write %s error!!\n", filename); return; }
    ...
    for (j=0; j<CAMNUM; j++) {
        ...
        for (int r=1; r<ART_RADIAL; r++, k++) {
            ...
            while (high-low > 1) {
                ...
                if (QuantTable[middle] < src_ArtCoeff[i][j][p][r]) low = middle; else high = middle;
            }
    ...
    for (k=0, a=0; k<ART_COEF; k+=2, a++) {
        if (k+1 < ART_COEF)
            q4_ArtCoeff[i][j][a] = ( (q8_ArtCoeff[i][j][k] << 4) & 0xf0 ) | ( q8_ArtCoeff[i][j][k+1] & 0x0f );
        else
    ...
    for (j=0; j<CAMNUM; j++) {
        ...
        q8_cirCoeff[i][j] = (itmp>255) ? 255 : itmp;
    }
    ...
    for (j=0; j<CAMNUM; j++) {
        ...
        q8_eccCoeff[i][j] = (itmp>255) ? 255 : itmp;
    }
    ...
    for (k=0; k<FD_COEFF_NO; k++) {
        ...
        q8_FdCoeff[i][j][k] = (itmp>255) ? 255 : itmp;
    }
...
#ifdef HEADLESS_OSMESA
    if (!create_osmesa_context(WIDTH, HEIGHT)) return 1;
    init();
    reshape(WIDTH, HEIGHT);
    renderingFun();          // exits(0) when done
    destroy_osmesa_context();
    return 0;
#else
    glutInit(&argc, argv);
